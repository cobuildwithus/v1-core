...
| codex-gpt5-flow-recipient-index-helpers-2026-03-04 | Add recipient index helper view methods for indexer mapping-table lookup path | `src/interfaces/IFlow.sol`, `src/Flow.sol`, `test/flows/FlowRecipients.t.sol`, `agent-docs/exec-plans/active/COORDINATION_LEDGER.md` | add `IFlow.recipientCount`; add `IFlow.recipientIdAtIndex`; add `Flow.recipientCount` + `Flow.recipientIdAtIndex`; add helper tests in `FlowRecipients.t.sol` | Keep append-only recipient-index semantics so removed recipients remain addressable by index for snapshot/indexer reconciliation | 2026-03-04 |
