...
| --- | --- | --- | --- | --- | --- |
| codex-gpt5-roundfactory-round-clones-2026-03-04 | Convert round stack deployment to full initializer+clone flow (remove direct `new RoundSubmissionTCR()` and `new RoundPrizeVault(...)` from `RoundFactory`) | `src/rounds/RoundFactory.sol`, `src/rounds/RoundPrizeVault.sol`, `script/DeployGoalFactory.s.sol`, `test/**`, `agent-docs/exec-plans/active/COORDINATION_LEDGER.md` | add `RoundPrizeVault.initialize`; rename `RoundFactory` constructor signature to accept round implementations; delete direct `new` deployment paths in round factory; update constructor callsites/tests | Must preserve round stack wiring and one-time initialization semantics for clone deployments | 2026-03-04 |
