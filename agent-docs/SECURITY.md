...
- Validate `CommunityGoalRegistry` goal-treasury metadata against the goal revnet id, keep the hook's `routeSetter` and goal-registry reference fixed at initialization, and keep privileged registry backstop actions explicit and minimal.
