...
- On escrow close to `Failed` or post-activation `Expired`, `PremiumEscrow.slash(underwriter)` computes exposure-weighted slash amount using `budgetSlashPpm` and calls `UnderwriterSlasherRouter`.
