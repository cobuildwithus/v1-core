...
interface IBudgetTCR is IGeneralizedTCR, IBudgetController {
    ...
    error PREPARED_ALLOCATION_MECHANISM_REQUIRED();
    error INVALID_PREPARED_CHILD_FLOW_RECIPIENT_ADMIN(address childFlowRecipientAdmin, address expectedRecipientAdmin);
    error PREPARED_PREMIUM_ESCROW_REQUIRED();
    error INVALID_PREPARED_PREMIUM_ESCROW(address premiumEscrow);
    error UNDERWRITER_SLASHER_NOT_CONFIGURED();
    error UNDERWRITER_SLASHER_CONFIG_MISMATCH();
