...
contract GeneralizedTCRStorageV1 {
    ...
    struct Request {
        ...
        // Snapshotted parameters at submission time (protect pending requests from later governance changes).
        uint256 challengePeriodDuration; // Challenge period duration at submission.
        uint256 disputeTimeout; // Dispute timeout at submission.
        uint256 arbitrationCost; // Arbitration cost at submission.
        uint256 challengeBaseDeposit; // Challenge base deposit at submission (submission/removal specific).
    }
    ...
    /**
     * @notice Strategy controlling how the registration submission base deposit is handled.
     */
    ISubmissionDepositStrategy public submissionDepositStrategy;
    /**
     * @notice Submission deposits held per item.
     * @dev This is the escrowed "payment" / bond for escrow-type strategies.
     */
    mapping(bytes32 => uint256) public submissionDeposits;
    /**
    ...
    /**
     * @notice Time after which a disputed request can be resolved if the arbitrator does not rule.
     */
